...
S3_PREFIX="${S3_PREFIX:-dittofs-edge/}"

# ---------------------------------------------------------------------------
# Helpers
# ---------------------------------------------------------------------------
log() { echo "[dittofs-edge] $(date '+%H:%M:%S') $*"; }
...
[Unit]
Description=DittoFS Edge Server (BadgerDB + S3)
...
RestartSec=5
LimitNOFILE=65536
Environment=DITTOFS_LOGGING_LEVEL=DEBUG
Environment=DITTOFS_CONTROLPLANE_SECRET=dittofs-edge-secret-key-for-jwt-1234567890
Environment=DITTOFS_ADMIN_INITIAL_PASSWORD=dittofs-edge-admin-1234567890
...
# ---------------------------------------------------------------------------
ADMIN_PASSWORD="dittofs-edge-admin-1234567890"
...
log "Creating /export share with pin retention..."
dfsctl share create --name /export --metadata badger-meta --local local-payload --remote s3-payload --retention pin
