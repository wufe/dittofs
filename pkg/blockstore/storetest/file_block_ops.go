...
// FileBlockStoreFactory creates a fresh FileBlockStore instance for each test.
// The factory receives *testing.T so it can use t.TempDir() for stores
// that need filesystem paths and t.Cleanup() for teardown.
type FileBlockStoreFactory func(t *testing.T) blockstore.FileBlockStore

// RunFileBlockStoreConformance runs the full conformance test suite for
// FileBlockStore implementations. Each test gets a fresh store instance.
func RunFileBlockStoreConformance(t *testing.T, factory FileBlockStoreFactory) {
	t.Helper()
...
func testListLocalBlocks(t *testing.T, factory FileBlockStoreFactory) {
	store := factory(t)
	ctx := t.Context()
	...
	for _, b := range result {
		if b.State != blockstore.BlockStateLocal {
			t.Errorf("ListLocalBlocks() returned block %s with state %v, want Local", b.ID, b.State)
		}
	}
}

func testListLocalBlocksLimit(t *testing.T, factory FileBlockStoreFactory) {
	store := factory(t)
	ctx := t.Context()
...
func testListLocalBlocksOlderThan(t *testing.T, factory FileBlockStoreFactory) {
	store := factory(t)
	ctx := t.Context()
...
func testListLocalBlocksEmptyStore(t *testing.T, factory FileBlockStoreFactory) {
	store := factory(t)
	ctx := t.Context()
...
func testListRemoteBlocks(t *testing.T, factory FileBlockStoreFactory) {
	store := factory(t)
	ctx := t.Context()
...
func testListRemoteBlocksLimit(t *testing.T, factory FileBlockStoreFactory) {
	store := factory(t)
	ctx := t.Context()
...
func testListRemoteBlocksEmptyStore(t *testing.T, factory FileBlockStoreFactory) {
	store := factory(t)
	ctx := t.Context()
...
func testListFileBlocks(t *testing.T, factory FileBlockStoreFactory) {
	store := factory(t)
	ctx := t.Context()
...
func testListFileBlocksOrdering(t *testing.T, factory FileBlockStoreFactory) {
	store := factory(t)
	ctx := t.Context()
...
func testListFileBlocksMixedStates(t *testing.T, factory FileBlockStoreFactory) {
	store := factory(t)
	ctx := t.Context()

	// Create blocks in all 4 states for same file
	states := []blockstore.BlockState{
		blockstore.BlockStateDirty,
		blockstore.BlockStateLocal,
...
func testListFileBlocksEmptyStore(t *testing.T, factory FileBlockStoreFactory) {
	store := factory(t)
	ctx := t.Context()
